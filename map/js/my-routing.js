...
let routeInfo = {
  ...
  distance: () => {
    ...
    if (safe == true) {
      for (i = 1; i < AllMarkers.length; i++) {
        distanceMiles = AllMarkers[i].safeRoute.paths[0].distance / 1609.34;
        sum = Math.round((sum + distanceMiles) * 10) / 10;
      }
      return sum;
    } else {
      for (i = 1; i < AllMarkers.length; i++) {
        distanceMiles =
          AllMarkers[i].dangerousRoute.paths[0].distance / 1609.34;
        sum = Math.round((sum + distanceMiles) * 10) / 10;
      }
      return sum;
    }
  ...
  time: () => {
    let sum = 0;
    let route = "";
    let speedMPH = 12; //bike speed in miles per hour (12mph is 1 mile every 6 minutes)  --  10mph 1 mile every 5 minutes
    if (safe == true) {
      for (i = 1; i < AllMarkers.length; i++) {
        timeMinutes =
          (AllMarkers[i].safeRoute.paths[0].distance / 1609.34 / speedMPH) * 60;
        sum = Math.round(sum + timeMinutes);
      }
      return sum;
    } else {
      for (i = 1; i < AllMarkers.length; i++) {
        timeMinutes =
          (AllMarkers[i].dangerousRoute.paths[0].distance /
            1609.34 /
            speedMPH) *
          60;
        sum = Math.round(sum + timeMinutes);
  ...
  Display: function () {
    // alert(routeInfo.measureCO2(AllPOIs));
    document.getElementById("lbsValue").innerHTML =
      routeInfo.measureCO2() + " lbs";
    document.getElementById("milageValue").innerHTML =
      routeInfo.distance() + " mi";
    document.getElementById("minuteValue").innerHTML =
      "~ " + routeInfo.time() + " min";
    // document.getElementById("lbsTitle").innerHTML = " lbs CO2";
  },
};
...
AddMarker = async (e) => {
  ...
  marker.recount = function () {
    ...
    for (i = this.id; i < AllMarkers.length; i++) {
      //Re-number the marker id's and poi id's to match their position in the array
  ...
  if (AllMarkers.length > 1) {
    await calculateRoute(
      AllMarkers[AllMarkers.length - 2],
...
calculateRoute = async (startMarker, endMarker) => {
  try {
    ...
    await Promise.all([
      ...
      .catch((error) => {
        console.log(
          "something went wrong in the route generation of the safe route, dangerous route, or CO2 Estimate"
        );
      });
...
displayMarkerRoute = (marker) => {
  if (safe == true) {
    map.addSource(marker.source, {
      type: "geojson",
      data: marker.safeRoute.paths[0].points,
    });
  } else {
    map.addSource(marker.source, {
      type: "geojson",
      data: marker.dangerousRoute.paths[0].points,
    });
  }
...
function Isochrone() {
  ...
  this.generateIsochronePolygon = function () {
    if (IsochroneActive == true) {
      ...
      var pointStr =
        isoMarker.getLngLat().lat + "," + isoMarker.getLngLat().lng; // var pointStr = e.latlng.lat + "," + e.latlng.lng;
      ghIsochrone
        .doRequest({ point: pointStr, buckets: 3, time_limit: 1000 })
        .then(function (json) {
          ABC = json;
          console.log(ABC);
      ...
      alert(
        "i think i should delete this, this will only pop up if an error occurs."
      );
    }
  };
...
GraphHopperIsochrone.prototype.doRequest = function (reqArgs) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    var url =
      ...
      args.basePath +
      "?" +
      that.getParametersAsQueryString(args) +
      "&key=" +
      args.key;
...
geocoder.on("result", function (e) {
  ...
  document
    .getElementById("directionshere")
    .classList.add("directionsbuttonvisible");
  document.getElementById("directionshere").innerHTML =
    "Get Route to " + geocoder._typeahead.selected.text;
});
...
document
  .getElementById("dropCrosshairMarker")
  .addEventListener("click", AddMarker);
document
  .getElementById("undoCrosshairMarker")
  .addEventListener("click", UndoButtonClicked);
document
  .getElementById("directionshere")
...
document
  .getElementById("HUDSwitch")
  .addEventListener("click", ToggleSafeRouting);
